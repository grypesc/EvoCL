...
def get_transforms(resize, test_resize, pad, crop, flip, normalize, extend_channel, extra_aug="", ds_name=""):
    ...
    if "cifar" in ds_name.lower():
        ...
        trn_transform_list = [
             transforms.RandomAffine(degrees=(-30, 30), translate=(0.2, 0.2), scale=(0.9, 1.1), interpolation=InterpolationMode.BILINEAR),
    ...
    elif "cub200" in ds_name.lower() or "aircraft" in ds_name.lower() or "domainnet" in ds_name.lower():
        trn_transform_list = [
                transforms.RandomResizedCrop(224),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
